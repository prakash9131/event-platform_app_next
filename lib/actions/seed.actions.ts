...
// Exact events from Evently website
const eventlyEvents = [
  {
...
export async function seedEventlyEvents() {
  try {
    ...
    const allEvents = [...eventlyEvents];
    
    // Add events for categories that don't have any from Evently
    const eventlyCategoryNames = eventlyEvents.map(e => e.categoryName);
    ...
    missingCategories.forEach((category, index) => {
      allEvents.push({
        ...
        location: "Community Center",
        imageUrl: availableImages[index % availableImages.length], // Use variety of images
        startDateTime: new Date(Date.now() + (index + 1) * 24 * 60 * 60 * 1000), // Future dates
        endDateTime: new Date(Date.now() + (index + 1) * 24 * 60 * 60 * 1000 + 2 * 60 * 60 * 1000), // 2 hours later
